...
class AugmentVersionManager:
    ...
    def backup_current_version(self):
        ...
        try:
            ...
            # Update config
            self.update_config("backup_created", {
                "version": version,
                "timestamp": timestamp,
    ...
    def list_backups(self):
        ...
        for backup_dir in self.backup_dir.glob("augment_v*"):
            if backup_dir.is_dir():
                ...
                self.log(f"  📦 {backup_dir.name} - Version: {version}, Size: {backup_info['size_mb']:.1f}MB")
    ...
    def rollback_to_version(self, backup_name_or_path):
        ...
        try:
            ...
            # Update config
            self.update_config("rollback_completed", {
    ...
    def lock_version(self):
        ...
        try:
            ...
            # Update config
            self.update_config("version_locked", {
                "version": version,
                "timestamp": datetime.now().isoformat(),
    ...
    def unlock_version(self):
        ...
        try:
            ...
            # Update config
            self.update_config("version_unlocked", {
                "timestamp": datetime.now().isoformat(),
                "extension_path": str(current_ext)
            })
            ...
            if is_locked:
                try:
                    with open(lock_file, 'r') as f:
                        ...
                        self.log(f"🕐 Locked at: {lock_info.get('locked_at', 'Unknown')}")
                except Exception:
                    pass
        else:
            self.log("❌ No Augment extension found")
