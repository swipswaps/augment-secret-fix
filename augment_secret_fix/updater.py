            ...
            if backup_dir.is_dir():
                ...
                if manifest_file.exists():
                    try:
                        ...
                        self.log(
                            f"  📦 {backup_dir.name} - Files: {backup_info['file_count']}, Size: {backup_info['size_mb']:.1f}MB"
                        )
            ...
            if method == "snap":
                ...
                if shutil.which("snap"):
                    snap_result = subprocess.run(
                        ["snap", "list", "code"], capture_output=True, text=True
                    )
    ...
    def interactive_chat_restore(self):
        ...
        try:
            ...
            if 1 <= choice_num <= len(backups):
                ...
                confirm = (
                    input(f"Restore backup from {timestamp}? (y/N): ").strip().lower()
                )
                if confirm == "y":
