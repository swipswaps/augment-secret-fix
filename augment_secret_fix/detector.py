...
class AugmentSecretDetector:
    ...
    def setup_logging(self):
        ...
        handler = RotatingFileHandler(
            self.log_file, maxBytes=5 * 1024 * 1024, backupCount=3  # 5MB
        )
            ...
            for pattern in augment_patterns:
                for ext_dir in config_dir.glob(pattern):
                    if ext_dir.is_dir():
                        ...
                        self.log(
                            f"✅ Found Augment extension: {ext_dir.name} in {config_dir.name}"
                        )
    ...
    def analyze_processes(self):
        ...
        for proc in psutil.process_iter(["pid", "name", "memory_info"]):
            try:
                ...
                if any(
                    keyword in proc_name for keyword in ["augment", "vscode", "code"]
                ):
                    ...
                    ):  # High CPU usage process (normalized)
                        self.log(
                            f"🔥 High CPU process: {proc.name()} (PID: {proc.pid}) - CPU: {cpu_usage:.1f}%, Memory: {memory_mb:.1f}MB (Threshold: {normalized_threshold:.1f}%)"
                        )
    ...
    def detect_secrets_activity(self):
        ...
        try:
            ...
            if io_start and io_end:
                ...
                self.log(
                    f"💾 Disk I/O Delta (2s) - Read: {read_delta_mb:.1f}MB, Write: {write_delta_mb:.1f}MB"
                )

                # Configurable threshold for high activity
                if (
                    read_delta_mb > self.disk_threshold
                    or write_delta_mb > self.disk_threshold
                ):
                    secrets_indicators.append("high_disk_io")
                    self.log(
                        "⚠️ High disk I/O delta detected - possible secrets processing"
                    )
        ...
        try:
            ...
            if net_start and net_end:
                sent_delta_mb = (
                    (net_end.bytes_sent - net_start.bytes_sent) / 1024 / 1024
                )
                recv_delta_mb = (
                    (net_end.bytes_recv - net_start.bytes_recv) / 1024 / 1024
                )

                self.log(
                    f"🌐 Network I/O Delta (1s) - Sent: {sent_delta_mb:.1f}MB, Received: {recv_delta_mb:.1f}MB"
                )

                # Configurable threshold for high activity
                if (
                    sent_delta_mb > self.network_threshold
                    or recv_delta_mb > self.network_threshold
                ):
                    secrets_indicators.append("high_network_io")
                    self.log(
                        "⚠️ High network activity delta detected - possible secrets transmission"
                    )
    ...
    def generate_report(self, cpu_analysis, processes, secrets_indicators):
        ...
        if cpu_analysis["high_usage_detected"]:
            report["recommendations"].append(
                "High CPU usage detected - consider Augment version rollback"
            )

        if "high_disk_io" in secrets_indicators:
            report["recommendations"].append(
                "High disk I/O detected - check for secrets processing"
            )

        if "high_network_io" in secrets_indicators:
            report["recommendations"].append(
                "High network activity detected - monitor secrets transmission"
            )

        if len(processes) > 5:
            report["recommendations"].append(
                "Multiple Augment processes detected - consider process cleanup"
            )
...
def main():
    ...
    if os.getuid() == 0:
        ...
        print(
            "   Running as root exposes process information and broadens attack surface."
        )
...
Environment Variables:
    ...
    parser.add_argument(
        "--extensions-dir", type=str, help="Custom VSCode extensions directory path"
    )
