...
class TestAugmentSecretDetector(unittest.TestCase):
    ...
    def test_detect_secrets_activity_high_network_io(self, mock_sleep, mock_net_io):
        ...
        mock_end = Mock(
            bytes_sent=20 * 1024 * 1024, bytes_recv=15 * 1024 * 1024
        )  # 20MB sent, 15MB received
    ...
    def test_generate_report_structure(self):
        ...
        report = self.detector.generate_report(
            cpu_analysis, processes, secrets_indicators
        )
        ...
        self.assertTrue(
            any("High CPU usage detected" in rec for rec in report["recommendations"])
        )
...
class TestCLIArguments(unittest.TestCase):
    ...
    @patch(
        "sys.argv",
        ["augment_secret_detector.py", "--cpu-threshold", "90", "--duration", "60"],
    )
